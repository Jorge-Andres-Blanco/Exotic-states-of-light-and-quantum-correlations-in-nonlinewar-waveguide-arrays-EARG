...
def plot_fidelity(phi_1,phi_3,x_med,theta,C0,ts,amp_ts,projection_list,file_name):
    """
    This function is supposed to be used to plot the fidelity as a function of the coupling constant C0
    Makes a .csv file that has two columns (or appends to it if exists), the first one for c0
    and the second one for the fidelity.
    The idea is to call this function a lot of times to get enough data to make a decent plot.
    ...
    RETURNS:
    NONE
    Saves the maximum value of fidelity in the csv file. The values of the fidelity are obtained from comparing the
    calculated state with the target state, in this case, it compares with the target state. Compares states of the form:
    ...
    Makes a .csv file that has two columns (or appends to it if exists), the first one for c0
    and the second one for the fidelity. This csv file can be later used fot plotting.
...
_ts_{ts[0]}_{ts[1]}'#This is the name of the results folder
    ...
    #Edits the list, appends the fidelity if a better result is found
    for element in file_list:
        try:
            if element[0]=="fidelity_py" and calc_state_vs_eigen_target[0][0]>float(element[1]):
                element[1] = str(calc_state_vs_eigen_target[0][0])
                element.append(f"c0={C0}")
        except:
            pass
    ...
    #Creates a csv file that appends the fidelity obtained and the value of c_0 for later plotting
    with open(f"fidelity_vs_c0_phi1_{int(phi_1)}p{int((round(phi_1,2)-int(phi_1))*100)}\
_phi3_{int(phi_3)}p{int((round(phi_3,2)-int(phi_3))*100)}_x_{int(x_med)}p{int((round(x_med,2)-int(x_med))*100)}_ts_{ts[0]}_{ts[1]}.csv", "a") as data_file:
        csvwriter = csv.writer(data_file)
        csvwriter.writerow([C0,calc_state_vs_eigen_target[0][0]])
